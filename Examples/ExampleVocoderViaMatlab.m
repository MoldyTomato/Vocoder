OSC = 1;
if OSC~=1
    system('".\Vocoder.exe" -f ConfigCMD.txt');
else
    %system('".\Vocoder.exe" -f ConfigOSC.txt &');
    u = udp('127.0.0.1',1);
    fopen(u);
    %pause(10);
    oscsend(u,'/test','ss','LoadForPlay','test.wav');
    pause(1.0);
    oscsend(u,'/test','ss','Play','Start');
    pause(3.0);
    oscsend(u,'/test','ss','Vocoder','On');
    pause(3.0);
    oscsend(u,'/test','si','Channelamount',10);
    pause(5.0);
    oscsend(u,'/test','s','Shutdown');
    fclose(u);
end
